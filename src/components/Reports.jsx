...
const Reports = ({ onDataFetch }) => {
  ...
  // Generate years array for the current year and the prev 5 years
  ...
  const handleButtonClick = async () => {
    if (selectedMonth && selectedYear) {
        try {
          // Call findByDate function to get data for selected month and year
          ...
          if(data.length !== 0){
            ...
            toast.warning(`There is no data to display on ${months[selectedMonth - 1].label} ${selectedYear}. Try another date`);
          }
        } catch (error) {
